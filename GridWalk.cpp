...
struct Monkey {
	...
	Monkey() {
		memset(&(pos[0][0]), 0, 400 * 400 * sizeof(int));
		count = Move(0, 0);
	}
	...
	*/
	...
	bool calulateDigit(int x, int y){
	...
	*/
	int Move(int x, int y) {
		...
		sum += Move(x + 1, y); //traverses through all x in first quad
		...
		if (x > y)
			sum += Move(x, y + 1); // traverses through all y in first quad
		return sum;
	}
};
...
int main() {
	Monkey monkey;
	printf("The total number of points the monkey can reach are: %d\n", monkey.count);
	getchar();
	return 0;
}
